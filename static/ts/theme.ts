...
const DARK_GRADIENTS: GradientToken[] = [
...
export class AvatarFallback {
  ...
  static colorFor(name: string): [string, string] {
  ...
  static build(name: string, size = 40): HTMLElement {
  ...
  static apply(imgEl: HTMLImageElement, name: string): void {
    ...
    avatar.style.minWidth  = "100%";
    avatar.style.minHeight = "100%";
...
export class ProfileImageLoader {
  private static _thumb(url: string, w: number, h: number): string {
    ...
    if (url.includes("googleusercontent.com"))
      return url.replace(/=s\d+-c/, `=s${Math.max(w, h)}-c`);
    return url;
  }

  static load(imgEl: HTMLImageElement, rawUrl: string, name: string, size = 80): void {
    if (!imgEl || (imgEl as HTMLImageElement & { _profileLoaded?: boolean })._profileLoaded) return;
    ...
    tmp.onload = () => {
      if (!imgEl.parentNode) return;
      imgEl.src = optimized;
      (imgEl as HTMLImageElement & { _profileLoaded?: boolean })._profileLoaded = true;
      imgEl.classList.remove("prof-img-loading");
      imgEl.onerror = () => { if (imgEl.parentNode) AvatarFallback.apply(imgEl, name); };
    };
    tmp.onerror = () => {
      imgEl.classList.remove("prof-img-loading");
      AvatarFallback.apply(imgEl, name);
    };
    tmp.src = optimized;
  }

  static initAll(selector = "img[data-profile]"): void {
    document.querySelectorAll<HTMLImageElement>(selector).forEach((img) => {
      ProfileImageLoader.load(
        img,
        img.dataset.profile      ?? "",
        img.dataset.profileName  ?? "",
        parseInt(img.dataset.profileSize ?? "80", 10),
      );
    });
  }
}
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => ProfileImageLoader.initAll());
} else {
...
declare global {
  interface Window {
    ...
    DS_Avatar: typeof AvatarFallback;
    DS_ProfileLoader: typeof ProfileImageLoader;

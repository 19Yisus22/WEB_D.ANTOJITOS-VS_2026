...
def usuario_get_by_correo(correo: str) -> dict | None:
    return _single(_run(
        ...
        .eq("correo", correo.lower().strip()).limit(1)
    ))
...
def usuario_get_by_identifier(identifier: str) -> dict | None:
    ...
    u = usuario_get_by_correo(ident)
    if not u:
        u = usuario_get_by_username(identifier.strip())
    if not u:
        u = usuario_get(identifier.strip())
    return u

def usuario_buscar_por_nombre(nombre: str) -> dict | None:
    return _single(_run(
        supabase.table("usuarios").select("cedula")
...
def usuario_touch(cedula: str, ts: str) -> None:
    _run(supabase.table("usuarios").update({"ultima_conexion": ts}).eq("cedula", cedula))

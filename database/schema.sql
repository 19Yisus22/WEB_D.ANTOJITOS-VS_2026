...
CREATE TABLE pedidos (
    ...
    CONSTRAINT pedidos_cedula_fkey FOREIGN KEY (cedula) REFERENCES usuarios(cedula) ON DELETE SET NULL ON UPDATE CASCADE
);
...
CREATE TABLE facturas (
    ...
    CONSTRAINT facturas_cedula_fkey FOREIGN KEY (cedula) REFERENCES usuarios(cedula) ON DELETE SET NULL ON UPDATE CASCADE,
...
CREATE TABLE comentarios (
    ...
    CONSTRAINT comentarios_cedula_fkey FOREIGN KEY (cedula) REFERENCES usuarios(cedula) ON DELETE SET NULL ON UPDATE CASCADE
);
...
CREATE TABLE usuario_logros (
    id uuid NOT NULL DEFAULT gen_random_uuid(),
    cedula text NOT NULL,
    codigo_logro text NOT NULL,
    fecha_desbloqueado timestamptz DEFAULT now(),
    CONSTRAINT usuario_logros_pkey PRIMARY KEY (id),
    CONSTRAINT usuario_logros_cedula_fkey FOREIGN KEY (cedula) REFERENCES usuarios(cedula) ON DELETE CASCADE,
    CONSTRAINT usuario_logros_codigo_fkey FOREIGN KEY (codigo_logro) REFERENCES logros(codigo) ON DELETE CASCADE,
    CONSTRAINT usuario_logros_unique UNIQUE (cedula, codigo_logro)
);
...
BEGIN
    FOREACH _fn IN ARRAY _fns LOOP
        IF EXISTS (
            ...
            EXECUTE format(
                'REVOKE EXECUTE ON FUNCTION public.%I() FROM anon', _fn
            );
        END IF;
    END LOOP;
END;
...
REVOKE EXECUTE ON FUNCTION public.actualizar_total_pedido() FROM anon;
REVOKE EXECUTE ON FUNCTION public.actualizar_ultima_conexion() FROM anon;
REVOKE EXECUTE ON FUNCTION public.asignar_rol_cliente() FROM anon;
REVOKE EXECUTE ON FUNCTION public.calcular_subtotal_detalle() FROM anon;
REVOKE EXECUTE ON FUNCTION public.calcular_total_carrito() FROM anon;
REVOKE EXECUTE ON FUNCTION public.es_admin() FROM anon;
REVOKE EXECUTE ON FUNCTION public.gestionar_stock_pedido() FROM anon;
...
CREATE POLICY "usuario_logros_own" ON usuario_logros FOR SELECT TO authenticated USING (cedula = current_setting('app.user_cedula', true));
CREATE POLICY "usuario_logros_insert" ON usuario_logros FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "usuario_logros_all_anon" ON usuario_logros FOR ALL TO anon USING (CURRENT_USER IS NOT NULL) WITH CHECK (CURRENT_USER IS NOT NULL);
